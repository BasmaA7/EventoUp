      ...
      @method('PUT')
        <div class="bg-indigo-50 min-h-screen md:px-20 pt-6">
            <div class="bg-white rounded-md px-6 py-10 max-w-2xl mx-auto">
                <h1 class="text-center text-2xl font-bold text-gray-500 mb-10">Edit Event</h1>
                @if ($errors->any())
...
@endif
                <div class="space-y-4">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description"  value="{{$event->description}}" cols="20" rows="5" 
                    ...
                    <div class="mb-4">
                        ...
                        <input name="quantity" type="text" value="{{$event->quantity}}" name="quantity"
                            class="ml-2 outline-none py-1 px-2 text-md border-2 rounded-md w-full" />
                    </div>
                    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/flatpickr/dist/flatpickr.min.css">
                    <div class=" mt-20">
                        <lable class="text-lg font-serif">Choose Date</lable> <br>
                        <input name="date"  value="{{$event->date}}" class="ml-2 outline-none py-1 px-2 text-md border-2 rounded-md w-full"
                       type="date" placeholder="Select a date">
                    </div>
                    <div class="mb-4">
                        <label for="title" class="text-lg font-serif">Location :</label>
                        <input type="text"  value="{{$event->location}}" placeholder="Title" name="location"
                            class="ml-2 outline-none py-1 px-2 text-md border-2 rounded-md w-full" />
                    </div>
                    <button type="submit"
                        class="px-6 py-2 mx-auto block rounded-md text-lg font-semibold text-indigo-100 bg-indigo-600">CHANGE
                        </button>
                </div>
            </div>
        </div>
